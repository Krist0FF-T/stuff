...
static const int texC = 12, texR = 16;

static Texture2D textures[texC];

static void initTextures() {
    Image tileSheet = LoadImage("img/blocks.png"),
        img;
    ...
    for(int i=0; i<texC; i++) {
        ...
        for(int x=0; x<texR; x++) {
            for(int y=0; y<texR; y++) {
                ImageDrawPixel(&img, x, y, GetImageColor(tileSheet, i*texR+x, (texR-1)-y));
            }
        }
...
void drawCubeTextureFaces(Vector3 position, Faces faces, int type) {
    ...
    if(isFlippable(type)) {
        SetRandomSeed((unsigned int) (position.x+position.y+position.z));
